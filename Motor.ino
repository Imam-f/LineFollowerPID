...
bool isRotated(bool CW) {
  int a, b, b_trgt;
  if (!CW)
    b_trgt = (prev_b + 4) % 16; // Clockwise
  else
    b_trgt = (prev_b - 4) % 16; // Counter Clockwise

  if (b_trgt < 0)
    b_trgt += 16; // Avoid Negative Bearing
  compass.read();
...
void adjustCar(int adjustMode)
{
  ...
  Serial.println("\n adjustCar \n");
  if (!adjustMode)
  { // 0: Too Front/Behind For reading QR
  ...
  {                    // 1: Too Right/Left, Aligning with Main Line
...
int readQR()
{
  ...
  while (!doneRead && times_reading < 4)
  { // Limit QR Reading only 4 times
    delay(50);
    if (Wire.available() >= 2)
    {
